...
# チーム名に混入しがちな余計な文字列（リンク文言・記号など）
JUNK_SUBSTRINGS = ["この記事を見る", "続きを見る", "もっと見る", "…", "...", "※"]


def clean_team_name(name: str) -> str:
    """チーム名から末尾に混入した余計な文字列・記号を取り除く"""
...
def fetch_latest_article_html(pref_category_id: int) -> str | None:
    ...
    article = soup.select_one(
        "div.entry_body, div.article, article, div.entry-content, div.post-body, div.ently_body"
    )
    if article is None:
        # フォールバック: 極端に大きい（複数記事が混ざっている疑いのある）ブロックは除外し、
        # その中で一番情報量の多いブロックを本文とみなす
        MAX_REASONABLE_LEN = 6000
        candidates = [d for d in soup.find_all("div") if len(d.get_text()) <= MAX_REASONABLE_LEN]
        if not candidates:
...
def parse_games(article_text: str):
    ...
    for line in article_text.splitlines():
        ...
        for m in GAME_RE.finditer(line):
            ...
            team1 = clean_team_name(m.group("t1"))
            team2 = clean_team_name(m.group("t2"))
...
def send_discord_notification(webhook_url: str, pref: str, loser: str, winner: str,
    ...
    content = (
        f"@everyone 🔔 **{pref}大会** {round_name or ''}\n"
