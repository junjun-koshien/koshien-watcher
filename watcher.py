...
def load_target_schools() -> dict:
    ...
    # 「付」と「附」は同じ意味で使われるが、速報サイトによって表記が違うことがある
    # (例:「西日本短大付」⇔「西短大附」)。どちらの表記で来ても拾えるように、
    # 該当する学校名は自動的にもう片方の表記も追加登録しておく。
    for pref_schools in schools.values():
        for name in list(pref_schools.keys()):
            counts = pref_schools[name]
            if "付" in name:
                variant = name.replace("付", "附")
                pref_schools.setdefault(variant, counts)
            if "附" in name:
                variant = name.replace("附", "付")
                pref_schools.setdefault(variant, counts)

    # 「商業→商」「工業→工」「農業→農」のような省略表記のゆれも、
    # 学校名の末尾がこれらのパターンに一致する場合は自動的に両方登録する。
    SUFFIX_PAIRS = [("商業", "商"), ("工業", "工"), ("農業", "農")]
    for pref_schools in schools.values():
        for name in list(pref_schools.keys()):
            counts = pref_schools[name]
            for long_suf, short_suf in SUFFIX_PAIRS:
                if name.endswith(long_suf):
                    variant = name[: -len(long_suf)] + short_suf
                    pref_schools.setdefault(variant, counts)
                elif name.endswith(short_suf):
                    variant = name[: -len(short_suf)] + long_suf
                    pref_schools.setdefault(variant, counts)

    # 「〇〇第一」⇔「〇〇一」のような番号表記のゆれ（末尾の漢数字の前に「第」が
    # あるかどうか）も自動的に両方登録する。
    NUMERALS = "一二三四五六七八九十"
    for pref_schools in schools.values():
        for name in list(pref_schools.keys()):
            counts = pref_schools[name]
            if len(name) >= 2 and name[-1] in NUMERALS:
                if name[-2] == "第":
                    variant = name[:-2] + name[-1]  # 第一 -> 一
                    pref_schools.setdefault(variant, counts)
                else:
                    variant = name[:-1] + "第" + name[-1]  # 一 -> 第一
                    pref_schools.setdefault(variant, counts)

    # 「県立」「市立」「府立」「都立」のような設置者表記は、速報サイトでは
    # 省略されることが多いので、あれば消したバージョンも自動的に登録する。
    for pref_schools in schools.values():
        for name in list(pref_schools.keys()):
            counts = pref_schools[name]
            for admin_prefix in ("県立", "市立", "府立", "都立"):
                if admin_prefix in name:
                    variant = name.replace(admin_prefix, "")
                    if variant:
                        pref_schools.setdefault(variant, counts)
...
def fetch_game_remark(detail_url: str) -> str:
    """試合詳細ページから「備考」（コールド・延長・サヨナラなど）を取得する。なければ空文字。"""
    try:
        resp = requests.get(detail_url, headers=HEADERS, timeout=15)
        resp.raise_for_status()
        resp.encoding = "utf-8"
        soup = BeautifulSoup(resp.text, "html.parser")
        # 空行を消さずに保持する（「備考」の直後が空欄＝備考なし、を正しく判定するため）
        lines = [s.strip() for s in soup.get_text(separator="\n").splitlines()]
        for i, line in enumerate(lines):
            if line == "備考" and i + 1 < len(lines):
...
def send_discord_notification(webhook_url, pref, loser, winner,
    ...
    if detail_url:
        result_line = f"[{result_line}](<{detail_url}>)"
    content = (
        f"@everyone 🔔 **{pref}大会**\n"
        f"{result_line}\n"
        f"検知時刻: {now_str}"
...
def check_prefecture(pref, slug, target_schools, notified, webhook_url, dry_run=False):
    ...
    for g in games:
        ...
        # 「試合終了」だけだと味気ないので、詳細ページから「備考」(コールド・延長など)を拾う
        display_status = g["status"]
        if detail_url:
            remark = fetch_game_remark(detail_url)
            if remark:
                display_status = remark

        print(f"  [敗退検知] {pref}: {loser}（春{counts['spring']}回、夏{counts['summer']}回）"
              f"が {winner} に {score_l}-{score_w}（{display_status}） で敗退")
        if not dry_run:
            send_discord_notification(webhook_url, pref, loser, winner, score_l, score_w,
                                       counts["spring"], counts["summer"], display_status, detail_url)
